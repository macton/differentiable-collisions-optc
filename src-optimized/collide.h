 ...
 * per-batch working set. cp_collide_scratch_bytes() returns the bytes needed
 * for prim_count primitives; pass a buffer of at least that size, malloc-
 * aligned. NULL/too-small for prim_count > 0 sets every result to
 * CP_ERR_NO_CONVERGE (explicit, never a hidden allocation). */
...
/* Precomputed-shape pipeline (build stage + runtime).
 *
 * cp_vshapes is an opaque, self-contained per-shape table: it holds, for every
 * primitive, the validation status plus the fully prepared world-space solver
 * geometry (the per-pair shape build is done ONCE here, not at solve time). It
 * is a flat POD blob with no pointers, so it serializes by raw copy.
 *
 *   build stage:  cp_vshapes_create  -> blob -> shapes_optimized.bin
 *   runtime:      blob -> cp_vshapes_from_blob -> cp_collide_pairs_vshapes
 *
 * cp_collide_pairs_vshapes needs no raw primitives: everything the solver reads
 * is already in the table, so the timed runtime is the solve alone. */
typedef struct cp_vshapes cp_vshapes;

/* Build-stage table construction — the caller provides the memory; the library
 * allocates nothing. cp_vshapes_bytes() reports the bytes needed for prim_count
 * primitives (0 on size overflow); cp_vshapes_create builds the table into
 * `buf` (at least that many bytes, malloc-aligned) and returns it, or NULL if
 * `buf` is missing or too small. The caller owns and frees `buf`. */
...
/* Raw serialized form: returns a pointer to the table's bytes and their count.
 * The pointer aliases the caller's buffer. */
const void *cp_vshapes_blob(const cp_vshapes *shapes, size_t *nbytes);

/* View bytes produced by cp_vshapes_blob AS a table, in place — no allocation
 * and no copy. The caller owns `data` (e.g. the buffer it read the file into)
 * and must keep it alive while the returned table is used; there is nothing to
 * free. Returns NULL on a size/shape mismatch. */
const cp_vshapes *cp_vshapes_from_blob(const void *data, size_t nbytes);

void cp_collide_pairs_vshapes(const cp_vshapes *shapes,
                              const cp_pair *pairs, uint32_t pair_count,
                              cp_result *results);
